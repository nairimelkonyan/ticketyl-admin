...
const mutations = {
  [mutationTypes.ADD_ORGANIZATION] (state, data) {
    if (data.configuration) {
      for (var i in data.configuration) {
        state.configuration[i] = data.configuration[i]
      }
    }
  },
  [mutationTypes.ADD_SHOP] (state, shop) {
    state.shops = [shop]
  },
...
const getters = {
  ...
  getAdminUserName: state => {
    return state.personal.first_name || state.personal.last_name ? `${state.personal.first_name} ${state.personal.last_name}` : '[Unknown]'
  }
  /* merchant: state => {
