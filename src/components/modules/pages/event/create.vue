          ...
          .row
            .col-sm-6
              .setup-content
                .setup-body
                  p Event Setup
                  h2(v-if='!serie.translations.name') Main Details - New Event
                  h2.title-filled(v-if='serie.translations.name') {{ serie.translations.name | tktranslate("en") }}
                    tk-tools(style='margin-left: 16px;')
                  span.status draft
            .col-sm-6.text-right
              .setup-content
                .setup-body
                  tk-secondary-button(name='save as draft', @click="store")
                  tk-primary-button(name='next step' @click='nextStep')
                  tk-primary-button(name='Publish', @click="publish")
        #content
          .row
