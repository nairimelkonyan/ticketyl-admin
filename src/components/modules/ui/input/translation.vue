<template lang="pug">
  section.input-translation(v-if="merchant && merchant.allowed_languages && language")
    div
      tk-input-type-text.select(v-for="(item, index) in merchant.allowed_languages", :key="item.code", v-if="language.code === item.code", v-model="translations[item.code]", :name='name', :placeholder='placeholder', :maxlength='maxlength', @input="update")
      .dropdown
